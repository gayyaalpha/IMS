...
class Login extends Component {
    ...
    loginWithSocialite=(provider)=>{
        window.location.replace('auth/redirect/'+provider)
    }
                ...
                >
                    <div className="auth-form-light text-left p-5 animated fadeIn">
                        ...
                        <form className="pt-3" ref={c => { this.form = c }} onSubmit={this.onSubmitHandle}>
                            ...
                            <div className="text-center mt-4 font-weight-light"> <Link onClick={()=>{this.forgetPassword()}} className="text-primary">Forgot your password?</Link >
                            </div>
                            ...
                            <div className="text-center mt-4 font-weight-light"> Don't have an account? <Link to='/registration' className="text-primary">Create</Link >
                               ...
                               <br/>
                                ...
                                {['facebook', 'google', 'linkedin'].map((provider)=>{
                                    return <div><i className={`mdi mdi-${provider} menu-icon`}></i> <Link onClick={()=>this.loginWithSocialite(provider)} className="text-primary"> {provider}</Link > </div>
                                })}
                                </div>
                            </div>
                        </form>
