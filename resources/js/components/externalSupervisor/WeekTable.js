...
const WeekTable = () => {
    ...
    function checkStatus(weekDays) {
        ...
        const filteredArray = dayArray.filter((item) => item.status_id !== 3);
    ...
    function checkStudentStatus(weekDays) {
        ...
        // console.log({dayArray})
        const filteredArray = dayArray.filter((item) => (item.status_id !== 1 && item.status_id !== 2));
        console.log({filteredArray})
        const allStatusIdEqualTo1 = filteredArray.length === 0;
    ...
    const submitForReview=()=>{
        ...
        if(weeks && weeks[weekNumber]){
            ...
            setIsLoading(true);
            axios.post('/api/v1/externalSupervisor/submitForReview', {
                ids:dates,
                api_token: authUser.api_token,
                review:reviewRef.current.value,
                mark:'',
                supervisor_id:externalSupervisor?.id,
                student_id:student?.id,
                week_no:weekNumber
            })
                .then(response => {
        ...
        <table className="table table-striped table-bordered">
            ...
            {Object.keys(weeks).map(weekNumber => (
                <React.Fragment key={weekNumber}>
                    <tr style={{background:'#F3F2F1',border:'1px solid'}}>
                        ...
                        <th colSpan={2}> {(checkStatus(weeks[weekNumber]))?<button  onClick={()=>{openReviewModal(weeks,weekNumber)}} className="btn-submit" >Submit</button>:''}
            ...
            <Modal title={`Add Review for Week ${reviewObject?.weekNumber}`} isOpen={isOpenSubmitModal} onModalClose={()=>{setIsOpenSubmitModal(false)}}>
                ...
                <div className="modal-footer">
                    <button id="save-button" disabled={isLoading} onClick={()=>{submitForReview()}} className="save-button">{isLoading ? "Loading.." : "Submit"}</button>
                </div>
            </Modal>
