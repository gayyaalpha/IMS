...
function CodeForm(props) {
    ...
    const {code, dropdowns} = useSelector(state => state.commonReducer);
    ...
    useEffect(() => {
        document.title =  'New Code';
        myDispatch(setActiveComponent( 'NewCode'));
    }, [])
    ...
    useEffect(() => {
        if (code) {
            setState({
                ...
                data: code,
                ...code,
                // supervisor: {value: code?.supervisor?.id, label: code?.supervisor?.name, item: code?.supervisor}
            })
        }
    }, [code])
    ...
    const onCreateCode = (e) => {
        if (simpleValidator.current.allValid()) {
            ...
            axios.post('/api/v1/code/create', $(e.target).serialize())
                .then(response => {
                    setState({
                        ...state,
                        loading: false
                                ...
                                >
                                    <form className="edit-lead-form border" onSubmit={onSubmitHandle}>
                                        ...
                                        <div className="form-group">
                                            <ul className="nav nav-tabs nav-pills c--nav-pills nav-justified">
                                                <li className="nav-item">
                                                    <span className="nav-link btn btn-gradient-primary btn-block active">{'Add Code'}</span>
                                                </li>
                                            </ul>
                                        </div>
                                        <div className="form-group">
                                        ...
                                        <div className="form-group text-center">
                                            ...
                                            <Link to='/code/list' className="btn btn-inverse-secondary btn-md">Cancel</Link>
                                        </div>
                                    </form>
                                </LoadingOverlay>
                            </div>
