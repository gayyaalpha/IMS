...
function TypeForm(props) {
    ...
    const {type, dropdowns} = useSelector(state => state.commonReducer);
    ...
    useEffect(() => {
        ...
        myDispatch(setActiveComponent( 'NewType'));
    }, [])
    ...
    useEffect(() => {
        if (type) {
            setState({
                ...
                data: type,
                ...type,
                // supervisor: {value: type?.supervisor?.id, label: type?.supervisor?.name, item: type?.supervisor}
            })
        }
    }, [type])
    ...
    const onCreateType = (e) => {
        if (simpleValidator.current.allValid()) {
            ...
            axios.post('/api/v1/type/create', $(e.target).serialize())
                .then(response => {
                    setState({
                        ...state,
                        loading: false
                                ...
                                >
                                    <form className="edit-lead-form border" onSubmit={onSubmitHandle}>
                                        ...
                                        <div className="form-group">
                                            <ul className="nav nav-tabs nav-pills c--nav-pills nav-justified">
                                                <li className="nav-item">
                                                    <span className="nav-link btn btn-gradient-primary btn-block active">{'Add Type'}</span>
                                                </li>
                                            </ul>
                                        </div>
                                        <div className="form-group">
                                        ...
                                        <div className="form-group text-center">
                                            ...
                                            <Link to='/type/list' className="btn btn-inverse-secondary btn-md">Cancel</Link>
                                        </div>
                                    </form>
                                </LoadingOverlay>
                            </div>
