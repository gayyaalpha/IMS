...
function DepartmentForm(props) {
    ...
    const {department, dropdowns} = useSelector(state => state.commonReducer);
    ...
    useEffect(() => {
        document.title =  'New Department';
        myDispatch(setActiveComponent( 'NewDepartment'));
    }, [])
    ...
    useEffect(() => {
        if (department) {
            setState({
                ...
                data: department,
                ...department,
                // supervisor: {value: department?.supervisor?.id, label: department?.supervisor?.name, item: department?.supervisor}
            })
        }
    }, [department])
    ...
    const onCreateDepartment = (e) => {
        if (simpleValidator.current.allValid()) {
            ...
            axios.post('/api/v1/department/create', $(e.target).serialize())
                .then(response => {
                    setState({
                        ...state,
                        loading: false
                                ...
                                >
                                    <form className="edit-lead-form border" onSubmit={onSubmitHandle}>
                                        ...
                                        <div className="form-group">
                                            <ul className="nav nav-tabs nav-pills c--nav-pills nav-justified">
                                                <li className="nav-item">
                                                    <span className="nav-link btn btn-gradient-primary btn-block active">{'Add Department'}</span>
                                                </li>
                                            </ul>
                                        </div>
                                        <div className="form-group">
                                        ...
                                        <div className="form-group text-center">
                                            ...
                                            <Link to='/department/list' className="btn btn-inverse-secondary btn-md">Cancel</Link>
                                        </div>
                                    </form>
                                </LoadingOverlay>
                            </div>
