...
function InternalSupervisorForm(props){
    let { id } = useParams();
    ...
    useEffect(() => {
        if(props.isEdit){
            ...
            props.setActiveComponentProp('EditInternalSupervisor');
        }else{
            document.title = 'Add Supervisor';

            props.setActiveComponentProp('AddInternalSupervisor');
        }
    }, []);
            ...
            axios.post('/api/v1/supervisor/create', $(e.target).serialize())
                .then(response => {
                    ...
                    if (response.data.status == 'validation-error') {
                        ...
                        history.push('/internal-supervisor/list')
                    }
                })
                .catch((error) => {
                    console.log(error);
                ...
                >
                    <form className="edit-lead-form border" onSubmit={onSubmitHandle}>
                        ...
                        <div className="form-group text-center">
                            ...
                            <Link to='/internal-supervisor/list' className="btn btn-inverse-secondary btn-md">Cancel</Link>
                        </div>
                    </form>
                </LoadingOverlay>
              </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(InternalSupervisorForm)
